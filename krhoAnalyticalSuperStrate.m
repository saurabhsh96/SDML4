...
function [kroTE1, kroTM1, kroTM0] =  krhoAnalyticalSuperStrate(k0,lam,h,er)   
%     Assumptions
%     tan(k0h) = kz0h - npi
%     hd = lam./4
%     kro = 0
...
%     zeta0 = 120*pi;
    ...
    kz0TE1 = (k0./(pi.*er.*(2*hbar).^2)).*((2*pi.*hbar.*er + 1j)./(1 + (1./(2*pi.*hbar.*er).^2)));
%     Z0TE1 = (zeta0.*k0)./(kz0TE1);

    kz0TM0 = sqrt((1j.*k0)./(er.*h));
%     Z0TM0 = (zeta0.*kz0TM0)./k0;

    kz0TM1 = (k0./(4.*hbar)).*(1 + sqrt(1 + 8.*1j.*hbar./(pi.*er)));
